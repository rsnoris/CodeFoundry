...
(function () {
  ...
  function showQuestion() {
    ...
    const opts = q.options.map((opt, idx) => `
      <div class="quiz-option${userAnswers[currentQIdx] === idx ? ' selected' : ''}"
           onclick="selectAnswer(${idx})">${opt}</div>
    `).join('');
    ...
    el.innerHTML = `
      <div class="quiz-progress">
        ...
        <span>
          ...
          Level ${currentLevel} &nbsp;·&nbsp; ${pct}% Complete
        </span>
      </div>
  ...
  window.nextQuestion = function () {
    if (currentQIdx < activeQuestions.length - 1) { currentQIdx++; showQuestion(); }
  };

  window.prevQuestion = function () {
    if (currentQIdx > 0) { currentQIdx--; showQuestion(); }
  };
